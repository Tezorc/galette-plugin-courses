...
- **Verification des en-tetes de licence** : le coeur a retire `malukenho/docheader` de son `require-dev` ; Galette verifie desormais ses en-tetes avec sa propre commande, `bin/console galette:headers:check`. Le plugin la reutilise via `--directory` et `--header-file`. Le gabarit est `.licence-header` (l'ancien `.docheader` est supprime) : il contient le **texte nu** de l'en-tete, sans les delimiteurs `/**` / `*/` ni le prefixe ` * ` — la commande les ajoute elle-meme, et un gabarit qui les inclut produit un en-tete doublement commente. Marqueurs disponibles : `%START_YEAR%`, `%END_YEAR%`, `%OWNER%`, `%PROJECT%`, `%URL%`. Le plugin passe `--owner`, `--project-url` et `--start-year=2026` ; il n'utilise pas `%PROJECT%`, la casse du nom differant d'une ligne a l'autre (« Galette Courses plugin » puis « Galette Courses Plugin »). `--end-year` vaut l'annee courante par defaut : au changement d'annee, passer `bin/console galette:headers:check --fix` pour mettre a jour les en-tetes.

  **Portee** : `--directory` pointe sur la racine du plugin, donc la verification couvre `lib/`, `tests/`, les `*.php` racine, mais aussi `templates/**.twig`, `webroot/*.css`, `scripts/*.sql` et `lang/*.php` (`vendor/`, `node_modules/` et les fichiers caches sont exclus par la commande). C'est plus large que l'ancienne etape docheader, qui ne prenait que `lib/` et les `*.php` racine — d'ou les 25 fichiers dotes d'un en-tete lors de la migration. La commande adapte le style de commentaire au type : `{# … #}` en Twig, `/*! … */` en CSS, `-- …` en SQL.

  **Piege sur les `scripts/*.sql`** : `--fix` prend le commentaire descriptif de tete pour un en-tete perime et le **remplace**, ce qui detruit de la documentation (le bloc « POURQUOI CE SCRIPT EXISTE » du rattrapage, par exemple). Les six fichiers ont donc ete traites a la main : en-tete de licence, **ligne vide** (indispensable, elle termine le bloc d'en-tete pour l'analyseur), puis le commentaire d'origine. Ne jamais lancer `--fix` sur `scripts/` sans relire le diff.

  Le texte de licence contient `WARRANTY;`, or `Install::executeSql()` decoupe les scripts sur `;`. Sans danger : `remove_remarks()` supprime toute ligne commencant par `--` **avant** le decoupage. Verifie explicitement — apres nettoyage, aucun fragment ne contient `WARRANTY` et le premier est bien une instruction SQL.
...
- Schema : colonnes `valid_from DATE NULL` / `valid_to DATE NULL` sur
  ...
  ALTER sont livres en `upgrade-slot-season*.sql` a appliquer a la main).

- `Entity/Event.php` :
  - `slotAppliesOn(array $slot, string $date)` : **statique et pure**, c'est la
    regle unique partagee par l'entite, le generateur et les tableaux bruts
...
- PHP >= 8.3 (compatible 8.3, 8.4, 8.5). Le plancher est passe de 8.2 a 8.3 pour suivre le coeur : Galette 1.3 refuse de demarrer sous 8.2 (`Galette requires PHP 8.3 minimum`), donc le plugin ne pouvait de toute facon pas y tourner. `.php-cs-fixer.dist.php` suit avec `@PHP8x3Migration`. **La matrice de la CI ne lit pas `composer.json`** : le generateur (`galette/.github/actions/matrix-generator`) prend `.github/galette.json` du depot s'il existe, sinon son propre `versions.json`. C'est donc `php_min` dans `.github/galette.json` qui pilote les versions testees — a tenir synchronise avec `composer.json` a la main.
- MySQL / MariaDB
