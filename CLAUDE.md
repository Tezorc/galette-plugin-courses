...
- `doc/tuto-adherent.md` : tutoriel destine aux **adherents**, diffusable tel quel. A mettre a jour uniquement quand le **parcours membre** change (inscription, liste d'attente, desinscription, notifications, connexion) — pas pour une evolution cote gestion. Il ne duplique pas le mode d'emploi : celui-ci renvoie vers lui (Annexe A) au lieu de reecrire le parcours, precisement pour que les deux ne divergent pas.
  ...
  reporter quand le `.md` bouge : `tuto-adherent.html`, illustre, 11 pages, et
  `tuto-adherent-condense.html`, sans reconstitution d'ecran, 4 pages, pour une
...
galette-plugin-courses/
  ...
  scripts/upgrade-slot-active.sql       # Migration MySQL : colonne is_active sur les creneaux (Phase 78)
  scripts/upgrade-slot-active-pgsql.sql # Migration pgsql : idem (Phase 78)
  scripts/upgrade-slot-season.sql       # Migration MySQL : colonnes season_from/season_to sur les creneaux (horaires saisonniers)
  scripts/upgrade-slot-season-pgsql.sql # Migration pgsql : idem
  ...
  scripts/upgrade-pref-value-text.sql       # Migration MySQL : pref_value varchar(255) -> TEXT (les periodes de fermeture debordaient)
  scripts/upgrade-pref-value-text-pgsql.sql # Migration pgsql : idem
  ...
  doc/
    ...
    tuto-adherent-condense.html    # Version courte du meme tutoriel, 4 pages (entree de build-pdf.py)
...
- Composants Fomantic ajoutes dynamiquement : le coeur ne les lie qu'une fois, au chargement (`_bindFomanticComponents` dans `galette-main.bundle.min.js`, qui fait `$('.ui.dropdown:not(.navigation, .autosubmit), select').dropdown()`). Tout element injecte ensuite en JS doit rappeler `.dropdown()` / `.checkbox()` sur son sous-arbre, sinon il reste un controle natif au rendu different (cas vecu sur les listes jour/mois des creneaux ajoutes).
- Horaires saisonniers : un creneau (slot) porte deux gates cumulatifs, `is_active` (interrupteur manuel, Phase 78) et la saison `season_from`/`season_to` (bornes incluses, chacune nullable = pas de borne). **La saison se saisit en jour + mois, sans annee** : le formulaire propose deux listes, et les bornes sont stockees en DATE sous l'annee sentinelle `Event::SEASON_YEAR` = 2000 (bissextile, donc 29/02 stockable), jamais relue — tout passe par `monthDay()`. La saison est donc recurrente, elle rejoue chaque annee. Corollaire : une saison peut enjamber le 31 decembre (hiver 01/10 -> 31/03), donc `from > to` est un cas normal — ne jamais rajouter de validation "fin avant debut", elle casserait l'hiver. La regle unique est `Event::slotAppliesOn(array $slot, string $date)`, **statique et pure** : elle sert a l'entite, a `RecurrenceHandler` et aux tableaux bruts postes par le formulaire — indispensable car `storeSlots()` supprime/reinsere les lignes a chaque enregistrement, donc `id_slot` n'est pas stable. Le filtrage se fait **par date d'occurrence** dans `generateSessions()` ET `backfillMissingSlots()`. `realignSeasonalSessions()` tourne avant le backfill pour replacer les seances deja generees a l'ancien horaire (sinon le backfill doublerait la date), en ne touchant que celles sans inscription ni moniteur.
- Preferences du plugin : `galette_courses_preferences.pref_value` est un `TEXT` (il etait `varchar(255)`, migration `upgrade-pref-value-text.sql`). La contrainte vient des periodes de fermeture, serialisees en JSON dans **une seule** ligne de preference : ~50 octets par periode plus un libelle de 120 caracteres max, les accents comptant 6 octets une fois echappes en `\uXXXX`. Trois periodes libellees depassaient les 255 octets, MySQL non strict tronquait, le JSON devenait invalide et `getClosureDates()` renvoyait `[]` — toutes les fermetures disparaissaient sans erreur. Corollaire durable : `PluginPreferences::set()` avale l'exception et renvoie `false`, donc **tout appelant doit tester son retour** sous peine d'afficher un succes sur un echec d'ecriture.
- Langue des courriels : les chaines PHP des courriels passent par `CourseNotification::tr()`, jamais `_T()` directement — `tr()` traduit dans la langue de l'association (`pref_lang`) via une locale explicite, quel que soit le contexte appelant. Un endpoint cron n'a ni session ni `Accept-Language` : `I18n` retombe sur `DEFAULT_LANG` = `en_US`, donc tout `_T()` y renvoie le msgid anglais (corps du mail francais car issu de la BDD, pied de page anglais). Les 3 endpoints de `CronController` appellent en plus `applyAssociationLanguage()` pour basculer toute la requete. Ne jamais "corriger" ce point en reenregistrant des loaders de traduction : les catalogues du domaine `courses` sont deja lies par `Plugins::loadModuleL10N()`, seule la locale demandee etait fausse.
...
- Actions destructives reservees au super admin : `coursesDoSessionRemove` (supprime une seance et, par cascade FK, ses inscriptions / liste d'attente / moniteurs) et `coursesDoRegenerateSessions` (purge **toutes** les seances datees d'aujourd'hui ou plus tard, tous statuts confondus, puis relance la generation sur la definition courante de l'evenement). Aucune des deux n'envoie de courriel — c'est un choix, pas un oubli. Double gate : ACL `superadmin` dans `_define.php` **et** `denyUnlessSuperAdmin()` dans le controleur. Attention : `isAdmin()` est vrai pour le super admin (Galette pose les deux drapeaux a la connexion), donc un gate sur `isAdmin()` seul aurait ouvert l'acces a tout adherent `bool_admin_adh`. Les seances passees ne sont jamais purgees. Pour la recurrence, la regeneration repart de la derniere seance restante (donc passee) et retombe sur `initial_session_date` seulement si l'evenement n'a plus aucune seance — sans quoi `getNextStartDate()` renvoie `null` et rien n'est regenere.
- ACL : l'ACL de route dans `_define.php` est un **filtre grossier**, pas l'autorisation reelle. Toutes les actions de gestion d'une seance (`coursesDoSessionClose`, `Reopen`, `Cancel`, `Reactivate`, `SessionEdit`, `DoSessionCapacity`, `DoPromoteWaitlist`, `DoAssignInstructor`, `DoProxyRegister`…) sont declarees `member` **exprès** : depuis la Phase 43, un moniteur affecte a la seance doit pouvoir les exercer, et il n'est ni staff ni groupmanager. Le vrai gate est `denyUnlessSessionManager($sessionId, …)` dans le controleur (admin OU staff OU instructeur de **cette** seance). Ne jamais "durcir" ces routes vers `staff` en lisant le seul `_define.php` : cela recasserait les droits moniteur, sans qu'aucun test ne le signale. Les niveaux qui, eux, portent vraiment la decision : `coursesDoEventValidate` / `Reject` / `DoGenerateSessions` / `EventRemove` : `staff` ; `coursesRegistrations`, `DoMarkAttendance`, `DoWalkIn`, `DoVolunteerInstructor`, `SessionExportRegistrations`, `MailSession` : `groupmanager` ; `coursesStats` / `coursesPreferences` / `coursesDoPreferences` : `staff` ; `coursesDoRegenerateCronToken` / `coursesMailTemplates` / `coursesDoMailTemplates` / `coursesDoMailTemplateReset` : `admin` ; `coursesDoRegenerateSessions` / `coursesDoSessionRemove` : `superadmin`.
