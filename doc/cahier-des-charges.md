...
- PHP >= 8.3 (compatible 8.3, 8.4, 8.5). Le plancher est passe de 8.2 a 8.3 pour suivre le coeur : Galette 1.3 refuse de demarrer sous 8.2 (`Galette requires PHP 8.3 minimum`), donc le plugin ne pouvait de toute facon pas y tourner. C'est aussi de `composer.json` que le generateur de matrice de la CI tire la version minimale a tester, et `.php-cs-fixer.dist.php` suit avec `@PHP8x3Migration`.
- MySQL / MariaDB
